...
def open_internet_explorer
  if $watir_script
    ...
    if @params['version'] == '11.5' # Condition for edge browser
      require 'watir'
...
Then /^I should see the Account name$/ do
  step "I wait 1 second"
  unless @browser.text.include? @account_name
    fail("Did not find text #{@account_name}")
  end
end
