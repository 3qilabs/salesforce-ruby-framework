...
def generate_request(req_param, test_params)
  ...
  if req_param.include? '"PASSWORD"'
    $test_params["PASSWORD"] = $params['environment']['password']
  end
  ...
  if req_param.include? '"USERNAME"'
    $test_params["USERNAME"] = $params['environment']['username']
  end
  ...
  if req_param.include? '"CLIENT_ID"'
    $test_params['CLIENT_ID'] = $params['environment']['client_id']
  end
  if req_param.include? '"CLIENT_SECRET"'
    $test_params['CLIENT_SECRET'] = $params['environment']['client_secret']
  end
  if req_param.include? '"AUTHORIZATION"'
    $test_params['AUTHORIZATION'] = "Bearer " + @test_params['access_token']
  end
...
def validate_special_value(v, v2)
  ...
  case v
    ...
    when "STRING_ALPHA_NUMERIC"
      result = v2 && v2.is_a?(String) && v2 =~ /[a-zA-Z0-9]/
